...
class Projects{
	...
	private function limit($start, $by = 'date'){
		return "ORDER BY `".$by."` LIMIT ".$start.", ".self::LIMIT;
	}
	...
	*	@param int $start (common) - row to start at
	*/
	public function retrieve($category, $by = 'date', $start = 0){ //GET
		...
		$query .= $this->limit($start, $by); //add query end
