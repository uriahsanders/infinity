...
function uploadImageFile() { // Note: GD library is required for this function
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($_FILES) {
            ...
            // if no errors and size less than 250kb (cahnged to 500kb)
            if (! $_FILES['image_file']['error'] && $_FILES['image_file']['size'] < 500 * 1024) {
                if (is_uploaded_file($_FILES['image_file']['tmp_name'])) {
                    ...
                    if (file_exists($sTempFileName) && filesize($sTempFileName) > 0) {
						...
						$srtFileName =	$rnd . substr($sExt, 1,1);
...
$nr = $member->setUsrData((($_POST['id'] == 1)?"image":"banner"), $sImage);
header("Location: /user/#".$nr)
?>