...
class Projects{
	...
	*/
	...
	public function search($key, $start = 0){ //GET
		...
		$result = $this->sql->query("SELECT (`ID`, `projectname`, `creator`, `date`, `popularity`, `short`, `image`) 
	...
	public function create($projectname, $short, $description, $image, $video){ //POST
		$result = $this->sql->query("INSERT INTO `projects` 
	...
	public function delete($id){ //POST
		$result = $this->sql->query("DELETE FROM `projects` WHERE `ID` = ? AND `creator` = ?", $id, $_SESSION['ID']);
	}
	...
	public function comments($id, $start = 0){ //GET
		$result = $this->sql->query("SELECT * FROM `projects_comments` WHERE `projectID` = ? ORDER BY `date` LIMIT ".$start.", ".self::LIMIT, $id);
		$res = [];
		foreach($result->fetchAll(PDO::FETCH_NAMED) as $row){
			array_push($res, $row);
		}
	...
	public function join($projectID, $bool){ //POST
		...
		$result = $this->sql->query("UPDATE `projects` SET `members` = ? WHERE `ID` = ?", json_encode($members), $projectID);
	}
