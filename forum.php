<?php
		include_once("server/templates/top.php");
	?>
	<br><br>
	<div style="position:fixed;top:50%;padding:20px">
		<a class="fa fa-caret-left fa-5x green"></a>
	</div>
	<div style="position:fixed;top:50%;right:0;padding:20px">
		<a class="fa fa-caret-right fa-5x green"></a>
	</div>
	<div id="body">
		<br>
		<div style="margin-left:15%;position:fixed;width:100%;z-index:10">
			<button class="btn btn-success fa fa-plus"></button>&emsp;
			<div class="btn-group">
				<button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
					Infinity-Forum <span class="caret"></span>
				</button>
				<ul class="dropdown-menu primary-menu" role="menu">
					<li><a href="#">General</a></li>
					<li><a href="#">Infinity</a></li>
					<li><a href="#">World</a></li>
					<li><a href="#">Arts</a></li>
					<li><a href="#">Sciences</a></li>
					<li><a href="#">Mathematics</a></li>
				</ul>
			</div>
			&nbsp;<span class="fa fa-long-arrow-right p-color"></span>&nbsp;
			<div class="btn-group">
				<button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
					Board <span class="caret"></span>
				</button>
				<ul class="dropdown-menu primary-menu"role="menu">
					<li>
						<span>Top Threads:</span>
						<a href="">Thread 1</a>
						<a href="">Thread 2</a>
						<a href="">Thread 3</a>
						<a href="">Thread 4</a>
						<a href="">Thread 5</a>
						<a href="">Thread 6</a>
						<a href="">Thread 7</a>
						<a href="">Thread 8</a>
						<a href="">Thread 9</a>
						<a href="">Thread 10</a>
					</li>
				</ul>
			</div>
			&nbsp;<span class="fa fa-long-arrow-right p-color"></span>&nbsp;
			<div class="btn-group">
				<button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
					Thread <span class="caret"></span>
				</button>
				<ul class="dropdown-menu primary-menu" role="menu">
					<li>
						<a href="">Page 1</a>
						<a href="">Page 2</a>
						<a href="">Page 3</a>
						<a href="">Page 4</a>
						<a href="">Page 5</a>
						<a href="">Page 6</a>
					</li>
				</ul>
			</div>
		</div>
		<br><br><br>
		<div class="panel panel-default"style="width:70%;">
			<div class="panel-heading">
				<h3 class="panel-title">General <i class="fa fa-caret-down"></i></h3>
			</div>
			<div class="panel-body">
				<table class=""cellpadding="10"style="width:100%;height:100%">
					<thead>
						<tr>
							<th>Name</th>
							<th>Topics</th>
							<th>Posts</th>
							<th>Last Post</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
					</tbody>
				</table>
			</div>
		</div>
		<br>
		<div class="panel panel-default"style="width:70%;">
			<div class="panel-heading">
				<h3 class="panel-title">Infinity <i class="fa fa-caret-down"></i></h3>
			</div>
			<div class="panel-body">
				<table class=""cellpadding="10"style="width:100%;height:100%">
					<thead>
						<tr>
							<th>Name</th>
							<th>Topics</th>
							<th>Posts</th>
							<th>Last Post</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
					</tbody>
				</table>
			</div>
		</div>
		<br>
		<div class="panel panel-default"style="width:70%;">
			<div class="panel-heading">
				<h3 class="panel-title">World <i class="fa fa-caret-down"></i></h3>
			</div>
			<div class="panel-body">
				<table class=""cellpadding="10"style="width:100%;height:100%">
					<thead>
						<tr>
							<th>Name</th>
							<th>Topics</th>
							<th>Posts</th>
							<th>Last Post</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
					</tbody>
				</table>
			</div>
		</div>
		<br>
		<div class="panel panel-default"style="width:70%;">
			<div class="panel-heading">
				<h3 class="panel-title">Arts <i class="fa fa-caret-down"></i></h3>
			</div>
			<div class="panel-body">
				<table class=""cellpadding="10"style="width:100%;height:100%">
					<thead>
						<tr>
							<th>Name</th>
							<th>Topics</th>
							<th>Posts</th>
							<th>Last Post</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
					</tbody>
				</table>
			</div>
		</div>
		<br>
		<div class="panel panel-default"style="width:70%;">
			<div class="panel-heading">
				<h3 class="panel-title">Sciences <i class="fa fa-caret-down"></i></h3>
			</div>
			<div class="panel-body">
				<table class=""cellpadding="10"style="width:100%;height:100%">
					<thead>
						<tr>
							<th>Name</th>
							<th>Topics</th>
							<th>Posts</th>
							<th>Last Post</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
					</tbody>
				</table>
			</div>
		</div>
		<br>
		<div class="panel panel-default"style="width:70%;">
			<div class="panel-heading">
				<h3 class="panel-title">Mathematics <i class="fa fa-caret-down"></i></h3>
			</div>
			<div class="panel-body">
				<table class=""cellpadding="10"style="width:100%;height:100%">
					<thead>
						<tr>
							<th>Name</th>
							<th>Topics</th>
							<th>Posts</th>
							<th>Last Post</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
						<tr>
							<td><a>General Discussion</a></td>
							<td>67</td>
							<td>5,432</td>
							<td>By <a>Uriah Sanders</a> 3 days ago</td>	
						</tr>
					</tbody>
				</table>
			</div>
		</div>
	</div>
</div>
<br>
<?php
		include_once("server/templates/bottom.php");
	?>
<!-- Scripts -->
<script src="js/forum.js"></script>
<script>

</script>
<?php
		include_once("server/templates/end.php");
	?>